...
<button type="button" class="wusb-ver" data-url="../firmware/firmware_wireless_dongle_v0.3.4.bin" data-name="firmware_wireless_dongle_v0.3.4.bin"><div class="wusb-ver__head"><span class="wusb-ver__tag">v0.3.4</span><span class="wusb-ver__badge wusb-ver__badge--latest">Latest</span></div><div class="wusb-ver__title">Licensed multi-device</div><div class="wusb-ver__desc">Required for wireless firmware updates. On v0.3.3 and older the Wireless USB resets itself partway through an over-the-air update, so pendant / AutoDustBoot / Smart RGB LED updates stall and never finish. Pendant + AutoDustBoot + Smart RGB LED on the same Wireless USB. App-only update that keeps your activation. A Wireless USB must be activated with your Installation ID in ncSender before it will relay.</div></button>
</div>
...
<script type="module">
  ...
  // Download a version tile's firmware and make it the selected one. Shared
  // by the click handler and the auto-select below, so both paths behave
  // identically — including leaving nothing selected if the fetch fails.
  async function loadFirmware(btn, opts) {
    if (btn.classList.contains('is-loading')) return false;
    const url = btn.getAttribute('data-url');
    const name = btn.getAttribute('data-name');
    if (!url || !name) {
      if (!(opts && opts.quiet)) log('Firmware URL not resolved yet — try again in a moment.', 'err');
      return false;
    }
  ...
  versionBtns.forEach((btn) => {
    btn.addEventListener('click', () => loadFirmware(btn));
  });
  ...
  flashBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      // App-only update: write just the app partition (no full-chip erase) so
      // the device's stored activation/pairing (NVS) survives the upgrade.
      ...
      log('Writing firmware at 0x10000 (app only — your activation is preserved)…');
